...
const SPILL_RETRIEVE_HINT =
  /use read with offset\/limit, or grep this path/i;

function isOfficialSpillNotice(text: string): boolean {
...
export function markRetrieveRegistered(): void {
  retrieveRegistered = true;
}

export function resetRetrieveRegistered(): void {
...
function warnMissingNative(): void {
  if (loggedMissingNative) {
    return;
  }
  loggedMissingNative = true;
  console.error(
    `dsh-compressor: native addon unavailable (${process.platform}-${process.arch}); leaving content unchanged`,
  );
}
...
function hasCodeFenceSection(original: string): boolean {
