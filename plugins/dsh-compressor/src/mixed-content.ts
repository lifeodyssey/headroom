...
export function isMixedContent(content: string): boolean {
  return (
    Object.values(mixedContentIndicators(content)).reduce(
      (sum, hit) => sum + Number(hit),
      0,
    ) >= 2
  );
}

export function mixedContentIndicators(
  content: string,
...
function anyNonblank(lines: readonly string[], start: number, stop: number): boolean {
...
function hasValidJsonBlockWithText(content: string): boolean {
  const lines = content.split("\n");
  let scanCache: JsonScanCache | null = null;
...
export function splitIntoSections(content: string): ContentSection[] {
  ...
  while (i < lines.length) {
    ...
    while (i < lines.length) {
      ...
      if (
        CODE_FENCE_PATTERN.test(nextLine) ||
...
function cacheKey(index: number, inString: boolean, escaped: boolean): string {
